...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords)
	...
	if Config.Multichar then 
		self.license = 'steam'.. identifier:sub(identifier:find(':'), identifier:len()) 
	else 
		self.license = 'steam:'..identifier 
	end
	...
	function self.setAccountMoney(accountName, money, reason)
		...
		if not tonumber(money) then 
			print(('[^1错误^7] 尝试设置 ^5%s^0 玩家账户 ^5%s^0 错误值 -> ^5%s^7'):format(accountName, self.playerId, money))
			return
		end
		if money >= 0 then
			...
			if account then
				...
				print(('[^1错误^7]尝试设置无效账户 ^5%s^0 玩家 ^5%s^0!'):format(accountName, self.playerId))
			end
		else 
			print(('[^1错误^7] 尝试设置帐户 ^5%s^0 玩家 ^5%s^0 错误值 -> ^5%s^7'):format(accountName, self.playerId, money))
		end
	end
	...
	function self.addAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			print(('[^1错误^7] 尝试设置账户 ^5%s^0 玩家 ^5%s^0 错误值 -> ^5%s^7'):format(accountName, self.playerId, money))
		end
	end
	...
	function self.canCarryItem(name, count, metadata)
        if ESX.Items[name] then
            ...
            print(('[^3警告^7] 不合法的物品 ^5"%s"^7被使用了'):format(name))
        end
	end
	...
	function self.setJob(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			print(('[ESX框架] [^3警告^7] 无效的 ^5.setJob()^7 函数调用 调用者: ^5%s^7, 职业: ^5%s^7'):format(self.source, job))
		end
	end
