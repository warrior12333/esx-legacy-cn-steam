...
function onPlayerJoined(playerId)
  ...
  if identifier then
    if ESX.GetPlayerFromIdentifier(identifier) then
      DropPlayer(playerId,
        ('加载角色时出错!\n错误代码: identifier-active-ingame\n\n此错误是由此服务器上具有与您相同标识符的玩家造成的。确保您不是在同一个Rockstar帐户上玩游戏。\n\n你的R星标识符为: %s'):format(
          identifier))
    else
    ...
    DropPlayer(playerId,
      '加载角色时出错!\n错误代码: identifier-missing-ingame\n\n此错误的原因未知，找不到您的标识符。请稍后返回或向服务器管理团队报告此问题。')
  end
end
...
function createESXPlayer(identifier, playerId, data)
  ...
  if Core.IsPlayerAdmin(playerId) then
    print(('[^2信息^0] 玩家 ^5%s^0 已授予最高管理员权限^7.'):format(playerId))
    defaultGroup = "admin"
  else
    defaultGroup = "user"
  end
...
function loadESXPlayer(identifier, playerId, isNew)
  ...
  if ESX.DoesJobExist(job, grade) then
    ...
    print(('[^3警告^7] 忽略的无效职业 ^5%s^7 [job: ^5%s^7, grade: ^5%s^7]'):format(identifier, job, grade))
    ...
    if result.inventory and result.inventory ~= '' then
      ...
      for name, count in pairs(inventory) do
        ...
        if item then
          ...
          print(('[^3警告^7] 无效物品 ^5"%s"^7 (^5"%s^7")'):format(name, identifier))
        end
      end
    end
  ...
  if result.group then
    if result.group == "superadmin" then
      ...
      print("[^3警告^7] ^5superadmin权限组^7 已经无效, 自动修改为 ^5admin^7")
    else
  ...
  if result.position and result.position ~= '' then
    ...
    print('[^3警告^7] 数据库中用户(users)表中玩家默认坐标有误 ,自动使用备份坐标，请修复你的数据表')
  ...
  print(('[^2信息^0] 玩家 ^5"%s"^0 已连接到服务器. ID: ^5%s^7'):format(xPlayer.getName(), playerId))
end
...
if not Config.OxInventory then
  ...
  AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
    ...
    if not sourceXPlayer or not targetXPlayer or distance > Config.DistanceGive then
      print("[^3警告^7]检测到玩家作弊！: ^5" .. GetPlayerName(playerId) .. "^7")
      return
    end
