...
function ESX.RegisterCommand(name, group, cb, allowConsole, suggestion)
  ...
  if Core.RegisteredCommands[name] then
    print(('[^3警告^7] 命令 ^5"%s" ^7已经存在，这将会覆盖命令'):format(name))
  ...
  RegisterCommand(name, function(playerId, args, rawCommand)
    ...
    if not command.allowConsole and playerId == 0 then
      print(('[^3警告^7] ^5%s'):format(TranslateCap('commanderror_console')))
    else
...
function ESX.TriggerServerCallback(name, requestId, source,Invoke, cb, ...)
  if Core.ServerCallbacks[name] then
    ...
    print(('[^1错误^7] Server callback ^5"%s"^0 does not exist. Please Check ^5%s^7 for Errors!'):format(name, Invoke))
    print(('[^1错误^7] 服务端 Callback ^5"%s"^0 不存在，请检查 ^5%s^7 处!'):format(name, Invoke))
  end
end
    ...
    {json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade, xPlayer.group, json.encode(xPlayer.getCoords()),
     json.encode(xPlayer.getInventory(true)), json.encode(xPlayer.getLoadout(true)), xPlayer.identifier}, function(affectedRows)
      if affectedRows == 1 then
        print(('[^2信息^7]^5"%s^7"已保存'):format(xPlayer.name))
      ...
      parameters, function(results)
        if results then
          if type(cb) == 'function' then
            ...
            print(('[^2INFO^7] 玩家 ^5%s^7 %s 已保存(^5%s^7 ms)'):format(count, count > 1 and 'players' or 'player', ESX.Math.Round((os.time() - time) / 1000000, 2)))
          end
        end
      end)
  end
...
function ESX.RefreshJobs()
  ...
  for _, v in ipairs(jobGrades) do
    if Jobs[v.job_name] then
      ...
      print(('[^3警告^7] 无效职业(工作无效) ^5"%s"'):format(v.job_name))
    end
  end
  ...
  for _, v in pairs(Jobs) do
    if ESX.Table.SizeOf(v.grades) == 0 then
      Jobs[v.name] = nil
      print(('[^3警告^7] 无效职业(职业等级无效) ^5"%s"^0'):format(v.name))
    end
  end
    ...
    if itemCallback then
      ...
      if not success then
        return result and print(result) or
                 print(('[^3警告^7] 使用物品 ^5"%s"^7时出错! 这不是ESX导致的.'):format(item))
      end
    end
  else
    print(('[^3警告^7] 物品 ^5"%s"^7 不存在，但是有玩家使用了它！'):format(item))
  end
end
