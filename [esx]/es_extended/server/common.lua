...
MySQL.ready(function()
  ...
  for _, v in ipairs(jobGrades) do
    if Jobs[v.job_name] then
      ...
      print(('[^3警告^7] 无效职业(职业缺失) ^5%s'):format(v.job_name))
    end
  end
  ...
  for _, v in pairs(Jobs) do
    if ESX.Table.SizeOf(v.grades) == 0 then
      Jobs[v.name] = nil
      print(('[^3WARNING^7]无效职业(等级缺失)^5%s^0'):format(v.name))
    end
  end
  ...
  print('[^2提示^7] ESX ^5Legacy 1.9.0^0 initialized!')
  print('[^2提示^7] ESX ^5框架 1.9.0^0(中文版) 已安装!')
  print('[^2提示^7] 由 YCG Team 汉化')
  print('[^2提示^7] yc-epls.cn')
  print('[^2提示^7] https://gitee.com/ycgroup_dev/esx-legacy-steam')
  StartDBSync()
  StartPayCheck()
end)
